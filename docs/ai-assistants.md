...
- **Gate tutorials with `shouldShowTutorial()` and persist with `userData: 'true'`.** Call `shouldShowTutorial()` once at boot **before** creating any tutorial overlay. If it returns `false` (`getUserData()` is a non-empty string), do **not** show tutorial UI — the player has played before. On **every** `reportResult(...)` call, pass `userData: 'true'` so the next launch skips the tutorial. Use `createTutorialOverlay` from `@minit-games/sdk/ui` — see [docs/tutorials.md](./docs/tutorials.md).
- **Tutorials: gestures over text, `highlight` on important elements.** Lead with `highlight` + `showFinger` / `showSwipe` — not a modal pill. Use `highlight` to draw attention to the relevant button, tile, or target on every step. Only use `showPill` when a rule truly cannot be shown through gestures alone. Avoid text that restates what the finger already demonstrates.
- **Do not restyle tutorial elements.** All tutorial visuals use fixed SDK theme defaults. Pass only position and interaction options (`x`, `y`, `direction`, `gesture`, `from`/`to`, `onClose`, `delay`) — no `fontSize`, `glyph`, `color`, `pulseScale`, or `theme.js` edits unless the creator explicitly requests a custom look.
