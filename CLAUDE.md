...
`src/modules/tutorial/` hosts the first-play tutorial overlay: `shouldShowTutorial()` (gating — `?tutorial=1|true` force-shows and `?tutorial=0|false` force-hides regardless of state; without an override it shows for new players and hides only on a non-empty userData value — host-injected persisted state or the `?userData=` local-dev fallback) and `createTutorialOverlay()` (DOM overlay with finger / swipe / highlight / pill primitives; design tokens in `theme.js`). `package.json` marks it in `sideEffects: ["**/modules/tutorial/**"]` so bundlers don't tree-shake it away.
